...
#> df = data.frame(data)
# In this tutorial we will use R dataset
df.orig <- USArrests
...
### hclust
hc1 <- hclust(d, method = "complete") # using hclust
hc1b <- hclust(d,method="ward.D2") # using hclust and ward.D emthod
...
pltree(hc2, cex = 0.6, hang = -1, main = paste("completeDendrogram of agnes method complete, ac = ",round(ac["complete"],2)))

hc3 <- agnes(df, method="ward")
pltree(hc3, cex = 0.6, hang = -1, main = paste("Dendrogram of agnes method ward, ac= ",round(ac["ward"],2)))
...
clusters = 4 # specify desired number of clusters based on optimization analyses see lines 84-94
subgrp <- cutree(hc5, k=clusters)
...
rect.hclust(hc5,k=clusters,border=2) # Add box around clusters using hclust

#### using agnes()
hc6 <- agnes(df,method="ward")
cutree(as.hclust(hc6), k= clusters)

#### using diana()
hc7 <- diana(df)
cutree(as.hclust(hc7),k=clusters)
plot(as.hclust(hc7), cex=0.6)
rect.hclust(as.hclust(hc7),k=clusters,border=2)
...
hc.cut <- hcut(df, k=clusters, hc_func="hclust", hc_method="complete") #hcut computes hierarchical clustering and cut the tree # can specify desire function hclust, agnes, diana
...
# Conclusions: Analysis Choices
 ## dissimilarity measure (aglomerative, divisive)
 ## type of linkage (complete, single, average, ward)
 ## where to cut dendrograms (cluster optimization)

################################################################################# 
# K-means clustering
set.seed(20)
km.clusters <- kmeans(df, clusters)
fviz_cluster(km.clusters, df)
#> str(km.clusters) # inspect kmeans clusters


## Kmeans clustering visualized in a map using lat/long and clusters (not used but intersting)
###> library(ggmap)
###> 
